...
class UserController(val service: UserService) {
    ...
    fun update(
        ...
        return service.update(id, request.name!!)
            ?.let { ResponseEntity.ok(service.toResponse(it)) }
    ...
    fun phoneLogin(@Valid @RequestBody login: PhoneLoginRequest): ResponseEntity<Any> {
        val result = service.loginByPhone(login.phone!!, login.uuid!!)

        return when (result.status) {
            UserService.LoginStatus.SUCCESS -> ResponseEntity.ok(service.toPhoneLoginUserResponse(result.user!!))
            UserService.LoginStatus.CODE_SENT -> ResponseEntity.status(HttpStatus.ACCEPTED).body("Código de confirmação enviado via SMS para o número ${login.phone}.")
            else -> ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Erro ao processar login.")
        }
    }

    @PostMapping("/confirm")
    fun confirm(@Valid @RequestBody payload: PhoneConfirmRequest): ResponseEntity<Any> {
        return service.confirmUser(payload.phone!!, payload.uuid!!, payload.code!!)
    }
